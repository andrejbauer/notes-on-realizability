...
language, see Section~\ref{sec:simply-typed-lambda-calculus} for a
...
In Section~\ref{sec:simply-typed-lambda-calculus} we shall elaborate
...
from the basic combinators $\combK$ and $\combS$, we need to postulate
the existence of other combinators. The end result looks more like a
programming language and less like a mysterious ``algebra''.
...
defined in Section~\ref{sec:pcas}.


\subsection{Simply typed $\lambda$-calculus}
\label{sec:simply-typed-lambda-calculus}

The simply\footnote{The types are \emph{simple} because they do not
  include the \emph{dependent} types.} typed $\lambda$-calculus is to
the untyped $\lambda$-calculus as TPCA is to a PCA. 

The types of a simply typed $\lambda$-calculus are inductively
generated from a non-empty family of \emph{basic} types using the type
constructors $\times$ and $\to$.
