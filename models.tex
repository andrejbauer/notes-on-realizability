...
accepts an infinite sequence on its input tape, and eventually fills
every cell on the output tape with a non-blank symbol. Obviously such
a machine keeps working and never terminates. We envision that we
would let it work until it produces a sufficient amount of output.
Note that the requirement for the output tape to be write-once makes
it possible to tell when the machine has produced a given amount of
output. Had we allowed the machine to write to each output cell many
times, it could keep coming back and changing what it has already
written.
