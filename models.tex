...
$\overline{n}$, $\combSucc$, $\combPRec$, and the numerals are
\emph{suitable} for~$A$.
...
\end{equation*}
%
There is also a corresponding $\eta$-reduction, which we do not assume
...
The typed $\lambda$-calculus is a TPCA. The types are those of the
...
\ttnat$, and for each type~$t$, a constant $\combPRec_t$ of type $t
\to (\mathtt{nat} \to t \to t) \to \ttnat \to t$. We also add two
further reduction rules
%
\begin{equation*}
  \combPRec \, e_1 \, e_2 \, 0 = e_1
  \qquad\text{and}\qquad
  \combPRec \, e_1 \, e_2 \, (\combSucc \, e_3) =
  e_2 \, e_3 \, (\combPRec \, e_1 \, e_2 \, e_3).
\end{equation*}
%
...
To obtain an example of a (total) NR-TPCA we enrich G\"odel's~$T$ with
general recursion. For every type $t$ we postulate a constant
...
of type $\ttbool$, and a constant $\combIf_t$ of type $\ttbool \to t
\to t \to t$, for each type~$t$. These satisfy the equations
%
\begin{equation*}
...
Note that the primitive recursion operator $\combPRec_t$ is
expressible with general recursion, booleans and the predecessor
function, namely $\combPRec_t\,x\,f\,n = g\,n$ where $g : \ttnat \to
t$ is
%
\begin{equation*}  
  \combFix_{\ttnat \to t} \,
...
We may form a NR-TPCA, which is actually total, by taking $A_t$ to be
the closed terms of type~$t$, quotiented by the equivalence relation
$\sim_t$ generated by $\beta$-reduction, rules for pairing,
projections, primitive recursion, the conditional statement, and
...
$\combFix_\ttnat (\xtlam{n}{\ttnat}{\combSucc\,n})$ and $\combFix_\ttnat
(\xtlam{n}{\ttnat}{\combSucc\,(\combSucc\,n)})$ both fail to denote a
numeral and should be considered equal.
...
returns a ground value.\footnote{We could have used functions which
  map into Booleans.} The programming language PCF is the NR-TPCA
where $A_t$ is the set of closed terms of type~$t$ quotiented by
observational equivalence $\approx_t$.

From the viewpoint of design of programming languages our definition
of PCF is not very satisfactory because it does not tell us how to
\emph{execute} programs, only when they are equal.
