...
\begin{proof}
  ...
  desk.\footnote{You have to ignore the fact the several hundred
    ...
    tape. Also, modern compuers are really \emph{Von Neumann}
...
which we can define as iterated pairing:
%
\begin{align*}
  \code{[\,]} &= 0 \\
...
\begin{proof}
  ...
  \begin{equation*}
     A = \set{(a, i, j) \in \NN^{*} \times \NN^{2} \such
        ...
        \xall{\alpha}{(\cons{a}{\Baire} \cap \dom{f})}{
          f(\alpha)(i) = j
          }
        }
  \end{equation*}
  ...
  then $j = j'$ because there is $\alpha \in \cons{a'}{\Baire} \cap
  \dom{f} \subseteq \cons{a}{\Baire} \cap \dom{f}$ such that $j =
  f(\alpha)(i) = j'$. We define a sequence $\gamma \in \Baire$ as
  follows. For every $(a, i, j) \in A$ let
  $\gamma(\code{\seq{\cons{i}{a}}}) = j + 1$, and for all other
  arguments $n$ let $\gamma(n) = 0$. Suppose that
  $\gamma(\code{\seq{\cons{i}{a}}}) = j + 1$ for some $i, j \in \NN$
  and $a \in \NN^{*}$. Then for every prefix $a' \sqsubseteq a$,
  $\gamma(\code{\seq{\cons{i}{a'}}}) = 0$ or
  $\gamma(\code{\seq{\cons{i}{a'}}}) = j + 1$. Thus, if $(a, i, j) \in
...
\begin{proof}
  ...
  $\gamma(\code{\seq{\cons{i}{a_{i,j}}}}) = 2$ for all $i, j \in \NN$,
  ...
  %
  \begin{equation*}
    \alpha \in \bigcap_{i \in \NN} \cons{a_{i, j_i}}{\Baire} \subseteq U.
  \end{equation*}
  %
...
\end{equation*}
%
In the other direction, to every $G \in \Scott$ we assign a function
$\Lambda(G) : \Scott \to \Scott$, defined by
%
\begin{equation*}
  \Lambda(G)(B) = \set{n \in \NN \such
    \xusome{A \wayb B}{\code{(\code{A}, n)} \in G}
  }.
\end{equation*}
%
...
syntactic model in which computation is expresses as manipulation of
...
argument~$a$ to the function $\ulam{y}{a^2 + y^3}$.
...
is written as $t : A$. aTo computer scientists the idea of expressions
...
\begin{definition}
  ...
  \indexsee{partial!application}{application, partial in PCA}%
                                %
  \emph{partial} binary operation $\place \cdot \place : A \times
  A \parto A$. We usually write $x y$ instead of $x \cdot y$, and
  recall that application associates to the left. Furthermore, there
  must exist $\combK, \combS \in A$ such that, for all $x, y, z \in
  A$,
  %
  \begin{xalignat*}{3}
    \combK x y &\simeq x \;, &
...
\begin{proof}
  ...
  properties. See~\cite{longley:phd} for details.
\end{proof}

The meta-notation $\pcalam{x}{e}$ plays a role similar to that of
$\lambda$-abstraction in the untyped $\lambda$-caclulus. We need to be
careful with the ``$\beta$-rule'' because it is only valid in a
restricted sense, see~\cite{longley:phd}.
...
$\combS$. In doing so the notation $\pcalam{x}{e}$ is handy because it
...
$\lambda^{*}$-notation. For natural numbers we may use the
%
\index{combinator!Curry numeral}%
\index{Curry numeral}%
\label{sym:curry_numeral}%
%
\emph{Curry numerals} which are defined by $\overline{0} = \combI =
\combS \combK \combK$ and $\overline{n+1} = \combPair \, \combFalse \,
\overline{n}$. There exist elements
%
...
Turing machines, or more precisely their codes, form a PCA whose
application is defined as $m \cdot n = \pr{m}{n}$. Because $m \cdot n$
can be easily confused with multiplication, a popular alternative
notation for this algebra is $\set{m}(n)$, but we shall not use it.

\paragraph{Second Kleene Algebra.}

Type 2 Turing machines, or more precisely the Baire space~$\Baire$, is
a PCA whose application is defined as $\alpha \cdot \beta =
\fpr{\alpha}{\beta}$.
...
$\lambda$-calculus. The graph model contains a computable subalgebra
of c.e.~sets.
...
to subsume many models of computation. Nevertheless, they lack an
important feature which many programming languages have, namely
\emph{types} that force us to observe certain rules for well-formed
programs. In contrast, a PCA has no restriction whatsoever how
application is formed. What we are looking for is \emph{typed PCA},
which was defined by John Longley.

A \emph{type world} $\mathcal{T}$ is a non-empty family, whose
elements are called \emph{types}, equipped with two binary operations
$\times$ and $\to$. A \emph{typed partial combinatory algebra (TPCA)}
$A$ over a type world $\mathcal{T}$ consists of
%
\begin{enumerate}
...
\end{align*}
%
We usually omit the subscripts and write $x \, y$ for $x \cdot_{s,t} y$.
...
\subsection{Examples of TPCAs}
\label{sec:examples-tpcas}

More details will be filled in this section.

\paragraph{Partial combinatory algebras.}

Every PCA $A$ is a NR-TPCA with over the trivial type world
$\mathcal{T} = \set{\star}$ in which $\star \times \star = \star \to
\star = \star$ and $A_\star = A$.

\paragraph{Simply typed $\lambda$-calculus.}

Simply typed $\lambda$-calculus is a TPCA over the types generated
from a collection of basic types with operations $\times$ and $\to$.

\paragraph{G\"odel's $T$.}
...
\paragraph{PCF with a monad.}

Given a monad $m$, as in Haskell, we define a NR-TPCA $A$ by
taking $A_{s \to t} = A_s \to m\,A_t$. More details to follow.


\paragraph{Real-world programming languages}
...
formal mathematical definition, see~\cite{definition-SML}. Ocaml and
