...
language and give explicit stategies for computing with it.
...
typical programming language we cannot meaningully apply \emph{any}
...
\begin{theorem}[Combinatory completeness]
  ...
  such that $\defined{e'}$ and $e \cdot a \kleq \subst{e}{x \mapsto
    a}$ for all $a \in A_t$.
\end{theorem}
...
%
\begin{equation*}
  (\xtlam{x}{t}{e_1}) e_2 = \subst{e_1}{x \mapsto e_2}.
\end{equation*}
%
...
The programming language PCF was introducted by Gordon
...
PCF is an extension of typed $\lambda$-calculus. There are two ground
...
$\numeral{n}$, $\combTrue$, and $\combFalse$,
$\lambda$-abstractions, and pairs $(e_1, e_2)$ where $e_1$ and $e_2$
are programs.
Execution of a program is described with the immediate reduction
relation $e \oneto e'$, which you can think of ``$e$ evaluates to $e'$
in one step'', defined by the rules in Figure~\ref{fig:pcf}.
...
\begin{gather*}
  ...
  \qquad
  \axiom
  {(\xtlam{x}{t}{e})\,e_2 \oneto \subst{e}{x \mapsto e_2}}
  \\ \\
  \infer
  {e \oneto e'}
  ...
  \infer
  {e \oneto e'}
  {\combPrec\,e \oneto \combPrec\,e'}
  \qquad
  \axiom
  {\combPrec\,\overline{n+1} \oneto \overline{n}}
  \qquad
  \axiom
  {\combPrec\,0 \oneto 0}
  \\ \\
...
%
\begin{xalignat*}{2}
  \combSucc\,\overline{n} &= \overline{n+1}, &
  \combIsZero\,\numeral{0} &= \combTrue,\\
  \combPrec\,\overline{n+1} &= \overline{n},&
  \combIsZero\,\numeral{n+1} &= \combFalse,\\  
  \combPrec\,0 &= 0, &
  \combIf_t \, \combTrue \, e_1 \, e_2 &= e_1,\\
  \combFix_t \, e &= e \, (\combFix_t \, e), &
...
For every type $t$ we postulate a constant
$\combFix_t$ of type $(t \to t) \to t$ and the reduction rule
%
\begin{equation*}
  \combFix_t \, e = e \, (\combFix_t \, e),
\end{equation*}
%
where $e$ is an expression of type $t \to t$. This is more than what
is strictly required to get a NR-TPCA, whose definition only wants the
fixed-point combinators of the form $\combY_{s,t} = \combFix_{s \to
  t}$.

To get something that looks like a programming language we also add a
second ground type $\ttbool$, constants $\combFalse$ and $\combTrue$
of type $\ttbool$, a constant $\combIsZero$ of type $\ttnat \to
\ttbool$, and a constant $\combIf_t$ of type $\ttbool \to t \to t \to
t$, for each type~$t$. The constant $\combIsZero$ tests whether a
number equals zero,
%
\begin{equation*}
  \combIsZero\,\numeral{0} = \combTrue
...
\end{equation*}
%
while $\combIf$ is the conditional statement,
%
\begin{equation*}
  \combIf_t \, \combTrue \, e_1 \, e_2 = e_1
  \qquad\text{and}\qquad
...
\end{equation*}
%
For better readability we write $\combIf_t \, e_1\, e_2 \, e_3$ as
$\cond{e_1}{e_2}{e_3}$.

Note that the primitive recursion operator $\combRec_t$ is
expressible with general recursion, booleans and the predecessor
function, namely $\combRec_t\,x\,f\,n = g\,n$ where $g : \ttnat \to
t$ is
%
\begin{equation*}  
  \combFix_{\ttnat \to t} \,
  (\xtlam{g}{\ttnat \to t}{
    \xtlam{n}{\ttnat}{
      \cond{n = 0}{x}{f\,n\,(g \, (n-1))}}}).
\end{equation*}
%
So we could omit primitive recursion if we took predecessor as a
primitive operation, and that is how PCF is customarily defined.

We may form a (total) NR-TPCA by taking $A_t$ to be the closed terms
of type~$t$, quotiented by the equivalence relation $\sim_t$ generated
by $\eta$- and $\beta$-reduction, rules for pairing, projections,
primitive recursion, zero testing, the conditional statement, and
general recursion. However, this gives us a somewhat unsatisfactory
model because it fails to equate certain expressions. For example,
$\combFix_\ttnat (\xtlam{n}{\ttnat}{\combSucc\,n})$ and
$\combFix_\ttnat (\xtlam{n}{\ttnat}{\combSucc\,(\combSucc\,n)})$ both
fail to denote a numeral and should be considered equal since,
intuitively, they are both an infinite nesting of applications
of~$\combSucc$.

To remedy the situation, we define a coarser equivalence relation
$\approx_t$, called \emph{observational equivalence}:
%
\begin{equation*}
  e_1 \approx_t e_2 \iff
  \xall{f}{A_{t \to \ttnat}}{\all{n}{\NN}{f\,e_1 \sim_\ttnat \numeral{n} \iff
      f\,e_2 \sim_\ttnat \numeral{n}}}.
\end{equation*}
%
The idea is that $e_1$ and $e_2$ are equivalent if they cannot be
distinguished by ``probing'' $e_1$ and $e_2$ with a function that
returns a ground value. PCF is the NR-TPCA where $A_t$ is the set of
closed terms of type~$t$ quotiented by observational equivalence
$\approx_t$.

\paragraph{PCF as a programming language.}

From the viewpoint of computation our definition of PCF is not
satisfactory because it does not tell us how to \emph{execute}
programs. To remedy the situation, we we define a relation $e \oneto
e'$ on closed expressions which you can think of ``$e$ evaluates to
$e'$ in one step of execution''. The relation is given by the
following inference rules.
%



\subsection{Extensions of PCF}
