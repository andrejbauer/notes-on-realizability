...
innocuous-looking idea that the input be stored on a tape gives the
type 2 machine the ability to take into account the \emph{location}
where a piece of data is stored. In this section we consider a
different model of intinite computation, the \emph{graph model}
introduced by Dana Scott~\cite{graph-model}. The basic idea is to
replace infinite sequences of numbers by sets of numbers.
