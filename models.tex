...
A machine computes a partial function $f : \Baire \parto \Baire$,
where $\Baire$ is the \emph{Baire space} of infinite sequences of
numbers, as follows. We run the machines with $\alpha \in \Baire$
...
multivariate parital function $f : (\Baire)^k \parto \Baire$. The
...
A partial computable function $f : \Baire \parto \Baire$ may be
encoded by an element $\beta \in \Baire$ as follows. Suppose that,
given input $\alpha \in \Baire$, the machine which computes $f$ writes
$j$ to the $i$-th output cell after $k$ steps of computation. This
means that $f(\alpha)_i = j$. Because in~$k$ steps the machine
inspects at most the first $k$ input cells, it would have done the
same for any other input that agrees with $\alpha$ in the first $k$
terms. This gives us the idea that $\beta$ should carry bits of
...
$\pair{i, \pair{[n_0, \ldots, n_k]}}$ where $\pair{{-}, {-}}$ and
$[{-}]$ are coding functions for pairs and sequences,
respectively.\footnote{if $m$ cannot be so decoded, we set $\beta_m =
  0$.} Then simulate the machine for $k$ steps, starting with input
tape $n_0, \ldots, n_k, 0, 0, \ldots$. If the machine writes $j$ into
the $i$-th output cell during the simulation, set $\beta_m = j + 1$,
otherwise set $\beta_m = 0$.
...
\begin{equation*}
  ...
  m = \min\nolimits_m (\xsome{i,k}{\NN}{m = \pair{{i, \seg{\alpha}{k}}} \land
    \beta_m \neq 0}).
\end{equation*}
%
We denote the function $f$ encoded by $\beta$ by $\xfpr{\beta}$. The
map $\xfpr : \Baire \to (\Baire \parto \Baire)$ is the type 2 version
of the standard numbering~$\xpr$. We want to know which partial maps
are of the form $\xfpr_\beta$.

\begin{proposition}
  For a partial function $f : \Baire \parto \Baire$ there exists
  $\beta$ such that $f = \xfpr_\beta$ if, and only if, $f$ is
  continuous, with respect to the product topology of $\Baire$, and
  its domain of definition is a $G_\delta$-set.
\end{proposition}

\begin{proof}
  Recall that a partial function is continuous when it is continuous
  on its domain of definition, with the subspace topology. A
  $G_\delta$-set is a countable intersection of open sets.
...
\end{proof}
 ...
 If
$\beta$ is a computable sequence, then $\xfpr{\beta}$ is computable by
a type 2 machine. In fact,


There is a pairing operation $\pair{{-},{-}}$ on $\Baire$ which simply
interleaves two sequences $\alpha, \beta \in \Baire$:
%
\begin{equation*}
  \pair{\alpha, \beta} =
  \begin{cases}
    \alpha(m) & \text{if $k = 2 m$,}\\
    \beta(m) & \text{if $k = 2 m + 1$.}
  \end{cases}
\end{equation*}
%
