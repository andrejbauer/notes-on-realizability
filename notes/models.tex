...
\begin{exercise}
  Implement the \defemph{minimization combinator $\combMin$}, which satisfies, for all $f \in A$ and $n \in \NN$,
  %
  \begin{equation*}
    \combMin \, f = \numeral{n}
    \iff
...
The Baire space~$\Baire$, is a PCA $(\Baire, {\mid})$ whose application is $\alpha \mid \beta = \xfpr{\alpha}{\beta}$, as defined in~\eqref{eq:type2-encoding}. The combinators $\combK$ and $\combS$ exists by \cref{th:type-2-utm,th:type-2-smd}, analogously to the first Kleene algebra.
...
\begin{definition}
  ...
  \end{align*}
  % 
  We say that $\ttnat$, $\numeral{n}$, $\combSucc$, $\combRec$, and the numerals satisfying these properties are \defemph{suitable} for~$A$.
\end{definition}
...
\begin{definition}
  ...
  \end{equation*}
  % 
  Such a $\combFix_{s,t}$ is \defemph{suitable} for~$A$.
\end{definition}
...
\defemph{Expressions over $A$} are defined inductively:
%
\begin{enumerate}
...
Giving a mathematically precise definition of a practical programming language is a lot of work, as can be testified by the authors of Standard ML~\sidecite{MilnerR:defsml}. Luckily, for our purposes we only need to formalize a small core language that captures the ``essence''.

For example, we could define a call-by-value PCF that captures the purely functional aspects of ML~languages (SML, OCaml, F\#), and possibly incorporate in it    computational effects (exceptions, mutable state, input-output). 
The interested reader should consult the work of John Longley~\sidecite{longley99:_unify_typed_and_untyp_realiz,longley99:_match} to see examples of PCF extended with computational effects, and to learn about general models of computation~\sidecite{longley14:_comput} that subsume combinatory algebras.
