...
\begin{definition}
  ...
  A partial function $f : \Baire \parto \Baire$ is \defemph{(type 2) realized} by $\gamma \in \Baire$ when $f = \xfpr_\gamma$.
\end{definition}
...
\begin{exercise}
  There is a pairing function $\pair{{-}, {-}} : \Scott \times
  ...
  \end{equation*}
  % 
  Verifty that $\pair{{-}, {-}}$ is a homeomorphism between $\Scott \times \Scott$ and
  $\Scott$.
\end{exercise}
...
If~$e$ contains such a redex, we may replace it by $\subset{e_1}{x \subto e_2}$ to obtain a new expression~$e'$. We say that we performed a \defemph{$\beta$-reduction} and write
%
\begin{equation*}
  e \mapsto e'
\end{equation*}
...
\begin{equation}
  ...
  \begin{aligned}
    ...
    \Gamma(\sem{\vec{x} \ctx e_1} \, \vec{a}) (\sem{\vec{x} \ctx e_2} \, \vec{a}), \\
    \sem{\vec{x} \ctx \lam{y}{e}} \, \vec{a} &= \Lambda(b \mapsto \sem{\vec{x}, y \ctx e} (\vec{a}, b)).
  \end{aligned}
\end{equation}
%
...
%
\begin{equation*}
  \sem{x, y \ctx x \, y}(\Lambda(f), a) = f a,
\end{equation*}
%
from which it follows that $\Lambda$ is a section of $\Gamma$ because
%
\begin{equation*}
  \Gamma(\Lambda \, f) \, a =
  \sem{x, y \ctx x \, y}(\Lambda(f), a) = f a.
...
We seek a topological space~$D$ with a continuous section-retraction pair\sidenote{$D$ must be nice enough for $\mathcal{C}(D,D)$, equipped with the compact-open topology, to be an \emph{exponential} in the category of topological spaces and continuous maps.}
%
\begin{equation}
...
\end{equation}
%
A non-trivial topological space~$D$ which retracts to its own continuous function space $\mathcal{C}(D,D)$, as abbove, is called a \defemph{reflexive domain}.


The denotations of $\lambda$-expressions in a reflexive domain~$D$ are given by~\eqref{eq:semantics-lambda-calculus}, and $\beta$-reduction is valid thanks to $\Lambda$ being a section of $\Gamma$:
%
\begin{align*}
  \sem{\vec{x} \ctx (\lam{y} e_1) \, e_2} \, \vec{a}
  &= \Gamma(\Lambda(b \mapsto \sem{\vec{x}, y \ctx e_1} \, (\vec{a}, b))) 
...
The retraction $\Gamma : D \to \mathcal{C}(D, D)$ decodes an element of~$D$ as a continuous map $D \to D$. We may also decode $a \in D$ as a continuous map $\Gamma^{(2)}\, a : D \times D \to D$ of two arguments, namely $\Gamma^{(2)}(a) = (\Gamma \, a) \circ p$ where $p : D \times D \to D$ is the above section.

It is now easy to state and prove the utm and smn theorems for reflexive domains. Note however that they are somewhat redundant because they just reduce to easy $\lambda$-calculus.

\begin{theorem}[reflexive domain smn]
  Given a reflexive domain~$D$,
  there is a continuous map $s : D \times D \to D$ such that $\Gamma (s(m, n)) \, a = \Gamma^{(2)} \, m \, (n, a)$ for all $m, n, a \in D$.
\end{theorem}
...
\begin{theorem}[reflexive domain utm]
  Given a reflexive domain~$D$, there is a continuous map $u : D \times D \to D$ such that $u (t, m) = \Gamma\,t\,m$ for all $t, m \in D$.
\end{theorem}
