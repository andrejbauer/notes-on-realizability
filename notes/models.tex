...
Earlier we encoded the partial computable maps $\NN \parto \NN$ with numbers.
We would like to similarly encode computable partial maps $\Baire \parto \Baire$ with 
sequences.

Consider a partial map $f : \Baire \parto \Baire$ computed by~$M$.
Suppose that, given input $\alpha \in \Baire$, $M$~writes $j$ to the $i$-th output cell after $k$~steps of computation, hence $f(\alpha)(i) = j$. Because in~$k$ steps~$M$ inspects at most the first $k$ input cells, it would have done the same for any other input that agrees with~$\alpha$ in the first~$k$ terms. This gives us the idea that~$f$ can be encoded by a $\gamma \in \Baire$ carrying bits of information of the form:
%
\begin{quote}
  ``If the input starts with $\seg{\alpha}{k}$ then $f(\alpha)(i) = j$.''
\end{quote}
%
More precisely, we let $\gamma(m)$ encode information as follows. Decode~$m + 1$ as
a finite sequence $\seq{i, n_0, \ldots, n_{k-1}}$ and simulate~$M$ for~$k$ steps with the input tape
%
\begin{equation*}
...
The above scheme ensures that $\gamma \in \Baire$ encodes a map $\xfpr_\gamma : \Baire \parto \Baire$, as follows. Define
%
\begin{equation*}
  \gamma \star \alpha = \gamma(\code{\seg{\alpha}{k}}) - 1
  \quad\text{where}\quad
  k = \min\nolimits_k (\gamma(\code{\seg{\alpha}{k}}) \neq 0)
\end{equation*}
%
(if not such $k$ exists then $\gamma \star \alpha$ is undefined), and
%
\begin{equation*}
  \gamma \mid \alpha =
  \begin{cases}
    \gamma \star (\cons{i}{\alpha}) & \text{if $\defined{\gamma \star (\cons{i}{\alpha})}$ for all $i \in \NN$,} \\
    \text{undefined} & \text{otherwise.}
  \end{cases}
\end{equation*}
%
The map encoded $\xfpr_\gamma : \Baire \parto \Baire$ by $\gamma$ is
%
\begin{equation}
  \label{eq:type2-encoding}
  \xfpr_\gamma(\alpha) = \gamma \mid \alpha.
\end{equation}
%
A partial map which equals $\xfpr_\gamma$ for some $\gamma \in \Baire$ is said to be \defemph{(type 2) realized} by~$\gamma$.

We would like to know which maps are realizable. A first observation is that $\xfpr_\gamma$ may not be computable when~$\gamma$ is non-computable. As it turns out, is $\xfpr_\gamma$ still reasonable from a  topological point of view.
...
\begin{proof}
  ...
  whose domain is a $G_\delta$-set. By \cref{th:extension_BB}
