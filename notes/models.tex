...
%
Because every $\lambda$-expression may be used either as an argument or a function, we shall require maps
%
\begin{equation}
...
\end{equation}
%
that mediate between the two roles. These can be used to give an interpretation
by assigning to each $\lambda$-expression~$e$ with free variables $x_1, \ldots, x_n$ a map
%
\begin{equation*}
...
\end{align*}
%
Now comes the trouble. By the $\beta$-rule~$\Lambda$ must be a section of~$\Gamma$,
%
\begin{align*}
  \Gamma(\Lambda(f)) \, a
  &= \Gamma(\sem{x \ctx x} (\Lambda(f), a)) \, (\sem{x \ctx x} \, a) \\
  &= \sem{x, y \ctx x , y}  \\
  &= \Gamma(b \mapsto (\sem{x \ctx x} \, f) \, b)
\end{align*}
