...
His definition is sets up a type system that limits the applicability of the basic basic combinators~$\combK$ and~$\combS$, so that existence of additional combinators is postulated. In the end we obtain a notion that is closer to a programming language than to magical incantations with~$\combK$ and~$\combS$.
...
$\numeral{n}$, $\combSucc$, $\combRec$, and the numerals are
...
\begin{proof}
  ...
  $\tpcalam{x}{t} e$ inductively as follows:
  %
  \begin{enumerate}
  ...
  \end{enumerate}
  %
  It turns out that $e' = \tpcalam{x}{t} e$ satisfies the conditions
  of the theorem.
\end{proof}
...
%
\begin{gather*}
  \star,\qquad \numeral{n},\qquad \combSucc,\qquad \combPred,\\
