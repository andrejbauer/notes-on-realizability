...
In information processing the open sets are not about geometry but about (positively)
observable properties. Thus it should not surprise you that the Scott topology is not
Hausdorff, not even a $T_1$-space. It is however a $T_0$-space, which is a form of Leibniz's principle of identity.\sidenote{The principle
  states that two things are equal if they have exactly the same properties.} We denote
$\pow{\NN}$ by $\Scott$ when we think of it as a topological space equipped with the Scott topology.
