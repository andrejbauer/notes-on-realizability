...
\begin{definition}
  ...
  \indexdef{simulation}%
  %
  A \defemph{simulation}, originally an \defemph{applicative morphism}~\cite{Longley:94},
  ...
  A \defemph{simulation} $\rho: (\EE, \subEE) \pcato (\FF, \subFF)$ between PCAs with subPCAs is a simulation
...
We take a simulation $\rho : \EE \pcato \EE$ to be a total relation rather than a function because there might be many ways to simulate an element of $\EE$ in $\FF$, without any one being canonical or preferred. The notation $\rho[u]$ suggests that~$\rho$ is construed as a multi-valued map rather than a mere relation.
%
...
PCAs with subPCAs and simulations therefore form a category, which is preoder enriched\sidenote{A category $\mathcal{C}$ is \defemph{preorder enriched} if hom-sets $\mathcal{C}(X,Y)$ are equipped with preorders (reflexive and transitive relations) under which composition is monotone.}
%
...
The preorder enrichment induces a notion of adjunction of simulations.
