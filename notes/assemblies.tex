...
\noindent
%
We sometimes indulge in writing the underlying set $\S{S}$ of an assembly~$S$ simply as~$S$.
...
Suppose $(\AA, \subAA)$ is an n-tpca. Let $N = (\NN, \ttnat, {\rz[N]})$ be the set of natural numbers~$\NN$ realized by the numerals, for all $r \in \Atyp{\ttnat}$ and $n \in \NN$,
%
\begin{equation*}
  r \rz[N] n \iff
  r = \overline{n}.
...
There are still more two-element assemblies, for instance the assembly of \defemph{semidecidable values} $\mathbb{S} = (2, {\rz[S]})$ given by
%
\begin{equation*}
  n \rz b \iff
...
\end{equation*}
%
Again, the identity map is realized as an assembly map $\two \to \mathbb{S}$, but not the other way around.

\begin{exercise}
  Why is identity not an assembly map $\mathbb{S} \to \two$?
\end{exercise}
...
as a map $\Ex_S : \S{S} \to \pow{\xAtyp{S}}$. The correspondence is
%
\begin{equation*}
  \R{x} \rz[S] x \iff \R{x} \in \Ex_S(x).
\end{equation*}
%
Because every $x$ is realized by something, $\Ex_S(x)$ always contains at least one element. Thus an assembly $\xasm{S}$ may be equivalently presented as a triple $(\S{S}, \T{S}, \Ex_S)$ where $\Ex_S : S \to \pow{\xAtyp{S}}$ is a map, called the \defemph{existence predicate}, such that $\Ex_S(x)$ contains at least one element for every $x \in \S{S}$. The name suggests that the elements of $\Ex_S(x)$ are computational witnesses for ``existence of~$x$''.

An assembly $S$ is modest if, and only if, $\Ex_S(x) \cap \Ex_S(y) \neq \emptyset$ implies $x = y$.

Under this formulation a map $f : S \to T$ is realized if there exists $\R{f} \in \subAtyp{\T{S} \to \T{T}}$ such that, for all $x \in \S{S}$ and $\R{x} \in \Ex_S(x)$, $\defined{\R{f}\,\R{x}}$ and $\R{f}\,\R{x} \in \Ex_T(f(x))$.
