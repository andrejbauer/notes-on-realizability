...
\end{equation*}
%
where $p \rz_P (x,y)$ if, and only if, $\combFst\,p
\rz_S x$ and $\combSnd\,p \rz_T y$.
...
\end{equation*}
%
where $o$ is any type.\sidenote{We need not specify $\rz_\zero$
...
\begin{equation*}
  ...
  \some{y}{T}{\R{y} \rz_T y \land y \equiv z}.
\end{equation*}
%
...
\begin{proof}
  ...
  \begin{equation*}
    p \rz_U u
    \iff \combFst\,p \rz_S f\,u \land \combSnd\,p \rz_S g\,u.
  \end{equation*}
  % 
...
\begin{align*}
  \asm{V} &= (\set{(x,y) \in T \times T \such x \equiv y},
             |T| \times |T|, {\rz_V}),\\
  p \rz_V (x,y) &\iff
  \combFst\,p \rz_T x \land \combSnd\,p \rz_T y, \\
  g (x, y) &= x,\\
  h (x, y) &= y.
\end{align*}
%
...
\item A primitive constant $b \in \asm{T}$ which is realized by $\R{b}
  ...
  which is realized\sidenote{Had we allowed as primitive constants
    ...
    constant map.} by $\tpcalam{x}{|S_1| \times \cdots \times
      |S_n|} \R{b}$.
