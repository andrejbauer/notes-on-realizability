...
\begin{equation*}
  \asm{S} \times \asm{T} =
  (S \times T, \T{S} \times \T{T}, {\rz[S \times T]})
\end{equation*}
%
...
\asm{T}$ are realized by $\R{f} \in \subAtyp{\T{U} \to \T{S}}$ and $\R{g}
...
%
\begin{equation*}
  \asm{P} = (\set{(x,y) \in S \times T \such f\,x = g(y)}, \T{S} \times
  \T{T}, {\rz[P]})
\end{equation*}
%
...
\begin{proof}
  ...
  %
  \begin{equation*}
    \asm{S} + \asm{T} = (S + T, \T{S} + \T{T}, \rz[S + T])
  \end{equation*}
  %
  ...
  T \to S + T$ are realized by $\combLeft_{\T{S},\T{T}}$ and
...
$f$ and $g$ computed in sets, as described above, $\T{\asm{T}/{\equiv}}
...
\begin{proof}
  ...
  \circ g = f \circ h$. Define the assembly $\asm{U} = (U, \T{S} \times
...
\subseteq T$, define the assembly $\asm{T}' = (T', \T{T}, {\rz[T']})$
...
\begin{proof}
  ...
  %
  \begin{align*}
    \asm{E} &= (\set{y \in T \such g(y) = h(y)}, \T{T}, {\rz[E]}), \\
...
the \defemph{quotient assembly} $\asm{T}/{\equiv} = (T/{\equiv}, \T{T},
...
\begin{proof}
  ...
  % $\all{y}{\asm{S}}{\some{x}{\asm{T}}{f\,x = y}}$. For the moment we
...
\begin{proof}
  ...
  \asm{T}/{\equiv}$ be realized by~$\R{f} \in \subAtyp{\T{S} \to \T{T}}$.
...
\iff \all{x}{P}{f\,x \leq_Q g\,x}$. The following definition explains
