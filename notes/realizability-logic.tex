...
Specifically, to each predicate $\phi$ on $S$ we assign its \defemph{characteristic function} $S \to \{0, 1\}$, defined by
%
\begin{equation*}
  x \mapsto
  \begin{cases}
...
\end{equation*}
%
as well as its \defemph{extension} $\set{x \in S \such \phi(x)}$. In fact, there is a bijective correspondence between predicates, characteristic functions, and subsets of~$S$.
...
\end{equation*}
%
commutes. Such an~$f$ is unique if it exists and is a mono.\sidenote{Such
...
The set $\Pred{S} = \pow{\Atyp{\T{p}}}^{\S{S}}$ of all realizability predicates on~$S$ is a preorder for the relation~$\entails$, defined as follows. Given $p, q \in \Pred{S}$, assert $p \entails q$ when there exists $\R{i} \in \subAtyp{\T{S} \to \T{p} \to T{q}}$, called a \defemph{witness}, such that whenever $\R{x} \rz[S] x$ and $\R{r} \rz p\,x$ then $\defined{\R{i}\,\R{x}\,\R{r}}$ and $\R{i}\,\R{x}\,\R{r} \rz q\,x$.
...
\begin{proof}
  ...
  The corresponding assembly map $i_p : S_p \to S$ is called a \defemph{extension} of~$p$.
...
We say that elements $p, q \in H$ are \defemph{equivalent}, written $p
