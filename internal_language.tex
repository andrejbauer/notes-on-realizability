...
the least equivalence relation~$\equiv$ satisfying, for all $x \in S$,
$f(x) \equiv g(x)$. The map $q : T \to T/{\equiv}$ is the canonical
...
%
\begin{equation*}
  \R{y} \rz_Q [z]_{\equiv} \iff
  \xsome{y}{T}{\R{y} \rz y \land y \equiv z}.
\end{equation*}
%
...
\begin{proof}
  ...
  \end{equation*}
  % 
  The maps $f$ and $g$ are morphisms from $\asm{U}$ to $\asm{S}$
...
\begin{proof}
  ...
  and the fact that an epi-mono in $\Set$ is the same thing as a
  bijection.
\end{proof}
...
is a regular mono because it is the equalizer of the maps $g, h : T
\to \nabla \two$, defined by
%
\begin{equation*}
  g(x) = 1
...
Given an assembly $\asm{T}$ and an equivalence relation $\equiv$
on~$T$, define the assembly $\asm{T}/{\equiv} = (T/{\equiv}, |T|,
\rz_{T/{\equiv}})$ of equivalence classes with $\R{x} \rz_{T/{\equiv}}
[y]$ if, and only if, $\R{x} \rz_T x$ and $x \equiv y$ for some $x \in
T$. The quotient map $q : T \to T/{\equiv}$ is realized and is a
coequalizer of $g, h : \asm{V} \to \asm{T}$ where
%
\begin{align*}
...
There is another characterization of regular epis which is usef often
by the TTE school.
...
\begin{proof}
  ...
  \asm{T} \to \asm{T}/{\equiv}$ be the coequalier of~$g$ and~$h$, as
  computed in Section~\ref{sec:cocartesian-structure}. Because~$f$ and
  $q$ both equalize $g$ and $h$, there exists a unique isomoprhism $i
