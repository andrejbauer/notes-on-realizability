...
respectively. A slight inconvenience arises when want to define a map
$f : S + T \to U$ by cases $f_1 : S \to U$ and $f_2 : T \to U$. One
possibility is
%
\begin{equation*}
  f(u) =
...
\end{equation*}
%
but this is seen rarely. In pratice mathematicians prefer to assume,
or shall we say \emph{pretend}, that the sets~$S$ and~$T$ are disjoint
and just write $S + T = S \cup T$. This allows us to write get rid of
the encoding by pairs,
%
\begin{equation*}
  f(u) =
  \begin{cases}
...
$y \in T$.\footnote{If you feel the urge to really encode everything
  ...
  $\iota_2(y) = (1, y)$ bu then forget the definition.} In a specific
case we may choose different, descriptive names for the injections.
Definition by cases is a primitive concept which deserves its own
notation, preferrably one that fits on a single line. A notation which
mimicks Haskell is
%
\begin{equation*}
...
In a general category the universal property a (binary) coproduct of
objects~$S$ and~$T$ is an object $C$ with morphisms $\iota_1 : S \to
C$ and $\iota_2 : T \to C$ such that, for all morphisms $f : S \to U$
and $g : T \to U$ there exists a unique $h : C \to U$ such that the
...
\begin{definition}
  ...
  \end{align*}
  %
  satisfying, for all $x$, $y$, $f$, $g$ of appopriate types,
  %
  \begin{align*}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \combIf_t &= \pcalam{\annot{b}{\ttbool}
                         \annot{x}{t}
                         \annot{y}{t}}{
                         \combRec_t \; y \; (\xpcalam{\annot{n}{\ttnat} \annot{z}{t}}{x})
                       }
  \end{align*}
  %
...
\end{equation*}
%
In the category of sets the coequalizer~$Q$ is the quotient of~$T$ by
the least equivalence relation~$\sim$ satisfying, for all $x \in S$,
$f(x) \sim g(x)$. The map $q : T \to T/{\sim}$ is the canonical
quotient map which takes $y \in T$ to its equivalence class
$[y]_{\sim}$.

In $\AsmA$ the coequalizer of $f, g : \asm{S} \to \asm{T}$ is the
assembly $\asm{Q} = \xasm{Q}$ where $Q$ is the coequalizer of $f$ and
$g$ computed in sets, as described above, $|Q| = |T|$, and $\rz_Q$ is
defined by
%
\begin{equation*}
  \R{y} \rz_Q [z]_{\sim} \iff
  \xsome{y}{T}{\R{y} \rz y \land y \sim z}.
\end{equation*}
%
The canonical quotient map $q : T \to Q$ is realized by
$\xpcalam{\annot{y}{|T|}}{y}$.
...
Recall that a morphism $f$ is a \emph{monomorphism (mono)} when it can
be canceled on the left: if $f \circ g = f \circ h$ then $g = h$. The
...
are precisely the injective and surjective maps, respectively. Let us
show that the same holds in the category of assemblies.

It is obvious that a morphism $f : \asm{S} \to \asm{T}$ is a mono if
it injective as a map $f : S \to T$. Conversely, suppose $f$ is a
mono. To show that it is injective, consider $x, y \in S$ such that
$f(x) = f(y)$. Let $\R{x}$ and $\R{y}$ be realizers for $x$ and $y$,
respectively. Define the assembly $\asm{U} = (\set{\star}, |S| \times
|S|, {\rz_U})$ where the only realizer for $\star$ is
$\combPair\;\R{x}\;R{y}$. Next, let $g, h : U \to S$ be the maps
$g(\star) = x$ and $h(\star) = y$. They are realized by $\combFst$ and
$\combSnd$, respectively. Because $f \circ g = f \circ h$ and $f$ is
mono, it follows that $g = h$, hence $x = g(\star) = h(\star) = y$.
This shows that $f$ is injective.

Next we consider epis. Again, it is easy to see that a morphism $f :
\asm{S} \to \asm{T}$ is epi if it is surjective. Conversely,
suppose~$f$ is epi. We show that it is epi as a map $f : S \to T$
in~$\Set$. Suppose $g, h : T \to U$ are maps in~$\Set$ such that $g
\circ f = h \circ f$. Both $g$ and $h$ are morphisms $g, h : T \to
\nabla U$ because they are realized by $\combK_{|S|,|\nabla U|} a$,
where $a \in \compAtyp{|\nabla U|}$. Since $f$ is epi in $\AsmA$, we
may cancel it and obtain $g = h$. This shows that $f$ is epi
in~$\Set$.
