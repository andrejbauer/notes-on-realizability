...
the interpretation of classical propositional calculus. If $\phi$ and
$\psi$ are propositions built from $\bot$, $\top$, $\land$, $\lor$,
$\lnot$, and variables $p$ ranging over $\pow{S}$,
%
\begin{align*}
  \sem{\bot} &= \emptyset,\\
  \sem{\top} &= S,\\
  \sem{\phi \land \psi} &= \sem{\phi} \cap \sem{\psi},\\
  \sem{\phi \lor \psi} &= \sem{\phi} \cup \sem{\psi},\\
  \sem{\lnot \phi} &= S \setminus \sem{\phi},\\
  \sem{p} &= p.
\end{align*}
%
The interpretation can be further extended to the classical predicate
calculus because $\pow{S}$ is \emph{complete}.
