...
$y \in T$.\footnote{If you feel the urge to really encode everything
  ...
  $\iota_2(y) = (1, y)$ bu then forget the definition.} We may even
choose descriptive names for the injections if we wish. Definition by
cases is a primitive concept which deserves its own notation,
preferrably one that fits on a single line. A notation which mimicks
Haskell is
%
\begin{equation*}
...
\begin{proof}
  ...
  $\defined{\combRight_{s,t}\,y}$ for all $x \in \typA{s}$, $y \in
  \typA{t}$. Next, because $\combCase_{s,t,u}$ realizes $h$,
...
The obvious question to ask is when a TPCA has sums.

\begin{proposition}
  A TPCA $\AA$ has sums if, and only if, it has \emph{booleans}, by
  which we mean that there is a type $\ttbool$, and for each type~$t$
  elements
  %
  \begin{equation*}
...
\begin{proof}
  Suppose $\AA$ has sums. Let $o$ be any inhabited type and $r \in
  \typA{o}$ and realizer in it, and define
  %
  \begin{align*}
    \ttbool &= o + o, \\
    \combTrue &= \combLeft\,r, \\
    \combFalse &= \combRight\,r \\
    \combIf_t &= \pcalam{\annot{b}{\ttbool}
                         \annot{x}{t}
                         \annot{y}{t}}
                        {\combCase_{o,o,t}\,b\,(\combK_{o,t}\,x)\,(\combK_{o,t}\,y)}
  \end{align*}
  %
  It is easy to check that these satisfying the required conditions.

  Conversely, suppose $\AA$ has booleans. For type $s$ and $t$ define
  %
  \begin{align*}
    s + t &= \ttbool \times (\ttbool \times s \to s) \times (\ttbool
    \times t \to t)
  \end{align*}
\end{proof}
