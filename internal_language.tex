...
set-theoretic complement $S \setminus A$. If we think of $A$ and $B$
as predicates on~$S$, these operations give us the interpretation of
logical connectives: truth $\top$ is $S$, falsehood $\bot$ is
$\emptyset$, conjunction $A \land B$ is $A \cap B$, disjunction $A
\lor B$ is $A \cup B$, negation $\lnot A$ is $S \setminus A$, and
implication $A \implies B$ is $(S \setminus A) \cup B$. Because
$\pow{S}$ is a Boolean algebra, the laws of classical propositional
