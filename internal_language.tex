...
$\lambda$-calculus. In addition if $\asm{T}$ is an assembly and $a \in
T$ is an element for which there exits a realizer $\R{a} \in
\compAtyp{|T|}$ then~$a$ is a primitive constant of type~$\asm{T}$.
...
\item A projection $\combFst\;e$ of type $\asm{T}$, where $e$ has type
  $\asm{T} \times \asm{U}$, is the map
  %
  \begin{equation*}
    \sem{\overline{x} : \overline{\asm{S}} \ctx
      \combFst\;e : \asm{T}}(a) =
    \pi_1 (\sem{\overline{x} : \overline{\asm{S}} \ctx e : \asm{T} \times \asm{U}}),
  \end{equation*}
  %
  which is realized by $\xpcalam{\annot{x}{|S_1| \times \cdots \times
      |S_n|}}{\combFst \; \R{e}}$, where $\R{e}$ is a realizer for
...
that any map defined by $\lambda$-calculus is realized, which is all
we need to know in most cases.
...
%
\begin{enumerate}
\item If $e$ has type $\asm{S}$ then $\combLeft_{S,T}\; e$ has type
  $\asm{S} + \asm{T}$.
\item If $e$ has type $\asm{S}$ then $\combRight_{S,T}\; e$ has type
  $\asm{S} + \asm{T}$.
\item If $e_1$ has type $\asm{S} + \asm{T}$.
\end{enumerate}
%
