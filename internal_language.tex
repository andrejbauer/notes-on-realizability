...
\end{equation*}
%
defined inductively as follows, where $a = (a_1, \ldots, a_n) \in S_1
\times \cdots \times S_n$:
%
\begin{enumerate}
...
\item A $\lambda$-abstraction $\xtlam{y}{\asm{U}}{e}$ of type $\asm{U}
  \to \asm{T}$ is interpred as follows. Let $f : \asm{S}_1 \times
  \cdots \asm{S}_n \times \asm{U} \to \asm{T}$ be the interpretation
  of $e$ in context $y : \asm{U}, \overline{x} : \overline{S}$. Then
  %
  \begin{equation*}
    \sem{\overline{x} : \overline{\asm{S}} \ctx
      (\xtlam{y}{\asm{U}}{e}) : \asm{U} \to \asm{T}} =
    \curry{f}
  \end{equation*}
  %
  where $\curry{f} : \asm{S}_1 \times \cdots \asm{S}_n \to
  \asm{T}^{\asm{U}}$ is the transpose of~$f$. The transpose
  $\curry{f}$ is realized because $f$ is realized by the induction
  hypothesis.
  %
\item The interpretation of an application $e_1\;e_2$, where $e_1$ has
  ...
  \end{equation*}
  %
  which is realized by $\xpcalam{\annot{x}{|S_1| \times \cdots \times
      |S_n|}}{(\R{e}_1\;x)\; (\R{e}_2\;x)}$, where $\R{e}_1$ and
  $\R{e}_2$ realize $\sem{\overline{x} : \overline{\asm{S}} \ctx e_1 :
    \asm{U} \to \asm{T}}$ and $\sem{\overline{x} : \overline{\asm{S}}
    \ctx e_2 : \asm{U}}$, respectively.
\item The interpretation of a pair $(e_1, e_2)$ of type $\asm{T}
  \times \asm{U}$ is the map
  %
  \begin{equation*}
    \sem{\overline{x} : \overline{\asm{S}} \ctx
      (e_1, e_2) : \asm{T} \times \asm{U}}(a) =
    (\sem{\overline{x} : \overline{\asm{S}} \ctx
      (e_1, e_2) : \asm{T}}(a),
    \sem{\overline{x} : \overline{\asm{S}} \ctx
      (e_1, e_2) : \asm{U}}(a)),
  ...
  %
  which is realized by $\xpcalam{\annot{x}{|S_1| \times \cdots \times
      |S_n|}}{\combPair \; (\R{e}_1\;x)\; (\R{e}_2\;x)}$, where $\R{e}_1$
  and $\R{e}_2$ realize $\sem{\overline{x} : \overline{\asm{S}} \ctx
    e_1 : \asm{T}}$ and $\sem{\overline{x} : \overline{\asm{S}} \ctx
    e_2 : \asm{U}}$, respectively.
\item A projection $\pi_1(e)$ of type $\asm{T}$, where $e$ has type
  $\asm{T} \times \asm{U}$, is the map\footnote{Do not confuse the two
    occurrences of $\pi_1$ in the equation. The one on the left-hand
    side is just a symbol appearing in the syntactic construct
    $\pi_1(e)$, while the one on the right-hand side denotes the first
    projection. If this does not make sense to you, imagine we wrote
    $\mathtt{barabado}(e_1)$ on the left-hand side. You would then
    want to know what this means, and the equation would tell you that
    $\mathtt{barabadoo}$ is the first projection.}
  %
  \begin{equation*}
    \sem{\overline{x} : \overline{\asm{S}} \ctx
      \pi_1(e) : \asm{T}}(a) =
    \pi_1 (\sem{\overline{x} : \overline{\asm{S}} \ctx
      e : \asm{T} \times \asm{U}}(a)),
  \end{equation*}
  %
  which is realized by $\xpcalam{\annot{x}{|S_1| \times \cdots \times
      |S_n|}}{\combFst \; (\R{e}\; x)}$, where $\R{e}$ is a realizer for
  $\sem{\overline{x} : \overline{\asm{S}} \ctx e : \asm{T} \times
    \asm{U}}$. The second projection $\combSnd$ is treated
  analogously.
\end{enumerate}
%
This rather long definition shows that we may freely use
$\lambda$-calculus to define realized maps. Although the definition
tells us exactly how to compute the realizers from the expressions,
the idea is to \emph{not} do that. We have verified once and for all
that any map defined by $\lambda$-calculus is realized, and in most
cases we do not care which specific realizer is used.
...
\item If $e$ is an expressions of type $\asm{S}$ then
  $\combLeft_{S,T}\; e$ is an expression of type $\asm{S} + \asm{T}$.
\item If $e$ is an expression of type $\asm{S}$ then
  $\combRight_{S,T}\; e$ is an expression of type $\asm{S} + \asm{T}$.
\item If $e_1$ has type $\asm{S} + \asm{T}$, $e_2$ has type $\asm{U}$,
  and $e_3$ has type $\asm{U}$, then
  %
  \begin{equation*}
    \case{e_1}{\combLeft\;x}{e_2}{\combRight\;y}{e_3}
  \end{equation*}
  %
...
\end{enumerate}
%
We usually omit the subscripts on $\combLeft$ and $\combRight$. The
expressions are interpreted as follows, with notation as above:
%
\begin{enumerate}
\item The interpretation of $\combLeft\;e_1$ is
  %
  \begin{equation*}
    \sem{\overline{x} : \overline{\asm{S}} \ctx
      \combLeft\;e_1 : \asm{S} + \asm{T}}(a) =
...
The proof by universal properties goes as follows:
%
\begin{equation*}
  ddd
\end{equation*}
