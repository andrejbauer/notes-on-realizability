...
\begin{proof}
  ...
  \to \Scott$ be the neighborhood filter pre-embedding from
  Theorem~\ref{thm:scott-embedding}. Define the assembly $F(X) =
  (X/{\equiv_X}, \Ex_{F(X)})$ by $\Ex_{F(x)} = \set{e_X(y) \such x
    \equiv_X y}$. To make $F$ into a functor we define $F(f) = f$ for
  a morphism $f : (X, {\equiv_X}) \to (Y, {\equiv_Y})$. If $f$ is
  tracked by $g : X \to Y$, then $F(f)$ is realized by a continuous
  extension of $e_Y \circ g : X \to \Scott$ along~$e_X$, which exists
  by the Extension Theorem~\ref{thm:scott-extension}.
...
\end{equation*}
%
The embeddings $e_X : X \monoto \Scott$ and $e_Y : Y \monoto \Scott$
are the neighborhood filters for~$X$ and~$Y$, respectively. We can
define a \emph{computable continuous map} $f: X \to Y$ to be a
...
This idea gives the following definition of computable continuous maps:
...
\begin{equation*}
  F' = \set{(\code{A}, m) \in \NN \times \NN \such
    \bigvee\nolimits_{B \subseteq A} (B, m) \in F
  }.
\end{equation*}
%
...
\begin{theorem}[Computable Extension Theorem]
  ...
  %
  Let $X$ and $Y$ be coutnably-based topological spaces and $f: X \to
...
TTE is traditionally expressed as a theory of representations, i.e.,
the category $\Rep{\Baire, \comp{\Baire}}$. There are actuallly three
variations:
%
\begin{enumerate}
...
\end{enumerate}
%
Mostly only the first two of these are used. Lately, multi-valued
representations have been considered also and for these we need to
move to the larger category of assemblies $\Asm{\Baire,
  \comp{\Baire}}$.
...
\begin{proof}
  ...
  a $T_0$-space, any~$\alpha$ enumerates the neighborhood filter of at
  most one~$x$, hence $\delta_X$ is single-valued. We leave
  admissibility of~$\delta_X$ as an exercise.
\end{proof}
