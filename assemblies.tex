...
\item $\Rep{\Baire, \Baire}$ is the \emph{continuous} version in which
  maps are realized by continuos realizers.
...
An equilogical space is a topological space\footnote{Originally Dana
  Scott restricted the definition to $T_0$-spaces.} with an
equivalence relation~\cite{BauerA:equs}. We show how this notions
is equivalent to assemblies over the graph model when we restrict to
countably-based spaces.

More precisely, a \emph{(countably-based) equilogical space} $(X,
{\equiv_X})$ is a countably-based space~$X$ with an equivalence
relation~$\equiv_X$. An \emph{extensional map} $f : (X, {\equiv_X})
\to (Y, {\equiv_Y})$ is a continuous map $f : X \to Y$ which respects
the equivalence relations: if $x \equiv_X y$ then $f(x) \equiv_Y
f(y)$. Extensional maps $f, g : X \to Y$ are \emph{equivalent},
written $f \equiv_{X \to Y} g$, when $x \equiv_X y$ implies $f(x)
\equiv_Y g(y)$. The category $\Equ$ has objects countably-based
equilogical spaces and morphisms are equivalence classes of
extensional maps.
...
\begin{proof}
  Here $\Scott$ is equipped with the Scott topology. If $X$ is
  pre-embedded in~$\Scott$ then it is countably based becase we may
  transfer the countable basis from~$\Scott$ to~$X$ along the inverse
  image of the pre-embedding.

  Conversely, suppose $(U_n)_{n \in \NN}$ is a countable topological
  basis for~$X$. Define the map $e : X \to \Scott$ by
  %
  \begin{equation*}
...
\begin{theorem}[Extension Theorem for $\Scott$]
  ...
  Suppose $e : X \to \Scott$ is a pre-embedding and $f : X \to Y$
  continuous. Then~$f$ has a continuous extension $g : \Scott \to Y$
  along~$e$.
\end{theorem}
...
The above two theorems now give us the desired equivalence.
